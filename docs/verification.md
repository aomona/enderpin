...
## 2026-09-25: ドキュメントサイト

- VitePressの本番ビルド、17 HTMLページ（404を含む）の内部リンク・見出しアンカー・参照アセットの検査に成功。
- Wrangler Static Assetsのローカル配信とデプロイ用dry-runに成功。npm auditは既知の脆弱性0件。
- ブラウザーで320px、1024px、1440px幅、ライト／ダーク表示を確認。ページ全体の横方向のはみ出しはありません。
- モバイル目次の開閉、Escapeで閉じる操作、日本語の「移行」検索と検索結果への移動を確認。日本語検索にはIntl.Segmenterによる単語分割を使います。
- 200%ズーム、スクリーンリーダー、OSのハイコントラスト設定を使った実機確認は未実施です。

今回はREADMEとサイトの変更です。ゲーム起動・各OSのサンドボックス動作を新たに検証したものではありません。
